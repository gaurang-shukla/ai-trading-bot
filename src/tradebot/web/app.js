...
async function home(){
  ...
  try{const data=await getJSON('/api/status');document.getElementById('layers').innerHTML=Object.entries(data.integrations).map(([key,item])=>{const ready=key==='weex'?item.configured:key==='openbb'?item.configured:item.installed&&item.configured;return `<div class="layer"><b>${key==='tradingagents'?'TradingAgents':key[0].toUpperCase()+key.slice(1)}</b><small class="${ready?'ready':'needs'}">${ready?'Configured':'Setup needed'}</small></div>`}).join('')}catch{document.getElementById('layers').innerHTML='<div class="layer"><small class="needs">Backend unavailable</small></div>'}
}
