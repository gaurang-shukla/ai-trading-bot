...
const chartLevelNames=[['Support'],['Resistance'],['Stop'],['Take profit'],['Current']];
function priceLineConfiguration(data,bars){
  const values=chartLevels(data),holding=data.risk_plan?.has_active_trade_setup===false;
  const levels=[[holding?'Watch below':'Support',values.support,'#24d395'],[holding?'Watch above':'Resistance',values.resistance,'#f7bd52'],['Stop',values.stop_loss,'#ff5b6f'],['Take profit',values.take_profit,'#559cff'],['Current',values.current_price,'#d8e5ef']];
...
function chartLevels(data){const plan=data.risk_plan||{},active=plan.has_active_trade_setup!==false;return {support:data.key_levels?.support,resistance:data.key_levels?.resistance,stop_loss:active?(plan.stop_loss??data.key_levels?.stop_loss):null,take_profit:active?(plan.take_profit??data.key_levels?.take_profit):null,current_price:data.live_price??data.market?.price}}
...
function signalPanel(data,label){const side=data.signal.side,plan=data.risk_plan||{},holding=plan.has_active_trade_setup===false;const market=location.pathname.split('/')[2]||'';const quote=market.startsWith('crypto_')?'USDT':'USD';const marketData=data.market||{};const liveTitle=market.startsWith('crypto_')?data.symbol.replace(/USDT$/,'/USDT')+' live price':data.display_name+' live price';const updated=data.last_updated?new Date(data.last_updated).toLocaleString():'—';const metrics=holding?[['Trade plan','No new trade'],['Stop loss','Not applicable for HOLD'],['Take profit','Not applicable for HOLD'],['Position size','0.0%'],['Risk score',plan.risk_score==null?'Not available':Math.round(plan.risk_score*100)+'/100']]:[['Stop loss',number(plan.stop_loss)],['Take profit',number(plan.take_profit)],['Position size',(plan.position_size_pct*100).toFixed(1)+'%'],['Risk score',plan.risk_score==null?'Not available':Math.round(plan.risk_score*100)+'/100']];return `<section class="detail-grid signal-price-grid"><article class="panel signal-card"><p class="eyebrow">${label}</p><div class="decision ${side.includes('BUY')?'positive':side.includes('SELL')?'negative':''}">${safe(side)}</div>${holding?'<p class="warning"><b>No active trade setup</b><br>HOLD means the app does not see a strong enough setup to open a new trade right now.</p>':''}<div class="live-price"><small>${safe(liveTitle)}</small><strong>${number(data.live_price??marketData.price)} <em>${quote}</em></strong><div><span class="${(data.change_24h??marketData.change_24h)>=0?'positive':'negative'}">24h ${pct(data.change_24h??marketData.change_24h)}</span><span>Volume ${money(data.volume??marketData.volume)}</span></div><p>Source: ${safe(data.source||marketData.source)} · Last updated: ${safe(updated)}</p></div><p>Confidence ${Math.round(data.signal.confidence*100)}%</p><div class="confidence-bar"><span style="width:${Math.round(data.signal.confidence*100)}%"></span></div>${metrics.map(x=>`<p><b>${x[0]}:</b> ${x[1]}</p>`).join('')}</article><article class="panel chart-card"><div class="chart-mount">${priceChart(data)}</div></article></section><section class="panel reasoning-panel"><p class="eyebrow">PLAIN-LANGUAGE REASONING</p><div class="plain-copy">${safe(data.plain_language_reason||data.signal.rationale)}</div></section>`}

function technicalPanels(data){const rows=data.timeframe_breakdown||[],levels=data.key_levels||{},plan=data.risk_plan||{},holding=plan.has_active_trade_setup===false;const planLevels=holding?`<div><small>Trade plan</small><strong>No active trade setup</strong></div><div><small>Watch above</small><strong>${plan.breakout_level==null?'Not available':number(plan.breakout_level)}</strong></div><div><small>Watch below</small><strong>${plan.breakdown_level==null?'Not available':number(plan.breakdown_level)}</strong></div><div><small>Current price</small><strong>${number(data.live_price??data.market?.price)}</strong></div>`:`<div><small>Support</small><strong>${levels.support==null?'Not available':number(levels.support)}</strong></div><div><small>Resistance</small><strong>${levels.resistance==null?'Not available':number(levels.resistance)}</strong></div><div><small>Stop loss</small><strong>${number(plan.stop_loss)}</strong></div><div><small>Take profit</small><strong>${number(plan.take_profit)}</strong></div>`;return `<section class="technical-grid"><article class="panel"><p class="eyebrow">MULTI-TIMEFRAME TECHNICALS</p>${rows.length?`<div class="table-wrap"><table class="market-table technical-table"><thead><tr><th>Timeframe</th><th>Trend</th><th>RSI</th><th>MACD</th><th>EMA bias</th><th>Score</th></tr></thead><tbody>${rows.map(r=>`<tr><td><b>${safe(r.timeframe)}</b></td><td class="${r.trend==='Bullish'?'positive':r.trend==='Bearish'?'negative':''}">${safe(r.trend)}</td><td>${number(r.rsi)}</td><td>${safe(r.macd)}</td><td>${number(r.ema_bias)}</td><td>${number(r.score)}</td></tr>`).join('')}</tbody></table></div>`:'<p class="warning">Candle history is temporarily unavailable. The signal safely fell back to live-price rules.</p>'}</article><article class="panel"><p class="eyebrow">KEY LEVELS &amp; RISK PLAN</p><div class="level-grid">${planLevels}</div></article></section><section class="panel why-panel"><p class="eyebrow">WHY THIS SIGNAL?</p><h2>${safe(data.trend_summary||'')}</h2><p>${safe(data.momentum_summary||'')}</p><p>${safe(data.volatility_summary||'')}</p><p class="plain-copy">${safe(data.plain_language_reason||data.signal.rationale)}</p></section>`}
